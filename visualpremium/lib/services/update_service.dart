...
class UpdateService {
  ...
  static Future<UpdateInfo?> checkForUpdates() async {
    try {
      ...
      if (data.containsKey('tag_name')) {
        ...
        _log('Última versão (GitHub): $latestVersion');
        ...
        if (exeAsset == null) {
          ...
          _log('Assets disponíveis: ${assets.map((a) => a['name']).toList()}');
          return null;
        }

        _log('Asset .exe: ${exeAsset['name']}');
        _log('URL download: ${exeAsset['browser_download_url']}');
        ...
        _log('Última versão: ${data['latestVersion']}');
        _log('URL download: ${data['downloadUrl']}');
        updateData = data;
      } else {
  ...
  // Retorna null em sucesso, ou String com a mensagem de erro
  static Future<String?> downloadAndInstallUpdate(
    ...
    try {
      ...
      _log('Caminho do instalador: $installerPath');

      final request = http.Request('GET', Uri.parse(downloadUrl));
      final streamedResponse = await request.send();

      _log('Status HTTP download: ${streamedResponse.statusCode}');
      ...
      _log(
        'Tamanho total: ${totalBytes > 0 ? '${(totalBytes / 1024 / 1024).toStringAsFixed(1)} MB' : 'desconhecido'}',
      );
      ...
      _log('Download concluído. Bytes recebidos: $downloadedBytes');

      if (downloadedBytes == 0) {
        return 'Download falhou: nenhum byte recebido.';
      }

      _log('Gravando arquivo em disco...');
      ...
      _log(
        'Arquivo gravado: ${(fileSize / 1024 / 1024).toStringAsFixed(1)} MB',
      );
      ...
      _log('Iniciando instalador silencioso...');
      ...
      _log('Encerrando app para instalação...');
      exit(0);
    } catch (e, stack) {
